...
- 나이: ${age}세
- 직업: ${occupation}
- 배경: ${userAnswers.answers[0] || '평범한 동물 친구'}를 동물 세계에 맞게 각색

**주요 고민 상황:**
사용자 고민: "${userAnswers.answers[2] || '일반적인 고민'}"에서 "${userAnswers.answers[3] || '구체적인 어려움'}"
→ 이를 ${characterName}의 동물 세계 일상 상황으로 완전히 변환하여 각색

**편지에 포함할 강점들 (동물 세계 맥락으로 비유하여 표현):**
${strengthsInfo}

**비유적 각색 요구사항:**
- 사용자의 실제 상황을 동물 세계의 다양한 생활 영역으로 완전히 변환
- 고민이 직업 관련이라면 동물 세계 직업 상황으로, 인간관계 관련이라면 동물 사회의 가족/친구/동료 관계로 각색
- 인간 세계의 용어나 상황을 직접 언급하지 말고 동물 세계 용어로 대체
- 예: "회사" → "숲속 작업장", "상사" → "족장", "동료" → "무리 친구들", "가족" → "가족 무리", "친구" → "숲속 친구들"

**상황별 각색 가이드:**
- **직업/업무 고민**: 동물 세계의 직업 상황과 작업장 관계로 각색
- **가족 관계 고민**: 동물 가족 무리의 관계와 갈등으로 각색
- **친구 관계 고민**: 숲속 친구들, 이웃 동물들과의 관계로 각색
- **연인 관계 고민**: 동물 세계의 짝짓기, 파트너 관계로 각색
- **사회적 관계 고민**: 동물 사회 전체의 소속감, 무리 내 지위로 각색
...
2. 동물 세계로 각색된 고민 상황 소개 - 강점 1개를 일상 특성으로 표현 (4-5문장)
3. 고민으로 인한 구체적 일상 어려움 - 동물 사회의 구체적 사건들로 묘사, 강점 1개 표현 (4-6문장)
4. 동물 무리나 가족/친구 관계에 미치는 영향을 구체적으로 묘사 - 강점 1개 표현 (4-5문장)
...
- 고민의 성격에 따라 적절한 동물 사회 상황들을 상세히 묘사 (직업, 가족, 친구, 연인 등)

**일관성 유지 필수 지침:**
- **전체 편지를 통해 하나의 일관된 캐릭터와 상황을 유지할 것**
- **${characterName}의 정체성과 동물 세계 설정을 처음부터 끝까지 일관되게 유지**
- **편지 전체에서 고민의 핵심 영역(직업/가족/친구/연인 등)을 흔들리지 않게 유지**
- **편지 중간에 다른 캐릭터나 다른 상황으로 바뀌지 않도록 주의**

**중요한 각색 지침:**
- 절대로 인간 세계의 직접적인 상황을 그대로 사용하지 말 것
- 동물의 본능과 특성을 활용한 자연스러운 비유로 변환
- 사용자 고민의 실제 영역(직업/인간관계/개인적 문제 등)을 파악하여 그에 맞는 동물 세계 상황으로 구성
...
- 사용자 고민의 실제 영역에 맞는 동물 세계 상황으로 자연스럽게 구성되었는지 확인
...
`;
    ...
    const completion = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: "system",
          content: "당신은 신경다양성을 가진 동물 캐릭터로서 비슷한 고민을 가진 사용자에게 도움을 요청하는 편지를 작성하는 전문가입니다. 편지는 충분히 길고 상세하며 감정적으로 깊이 있게 작성해야 합니다. 중요한 규칙: 1) 절대로 강점 이름을 직접 언급하지 말고 행동으로만 표현할 것 2) 편지 전체에서 하나의 일관된 캐릭터와 상황을 유지할 것 3) 동물 세계 설정을 끝까지 일관되게 유지할 것 4) 사용자 고민의 실제 영역(직업/가족/친구/연인 등)을 파악하여 적절한 동물 세계 상황으로 각색할 것"
        },
        {
          role: "user",
          content: prompt
